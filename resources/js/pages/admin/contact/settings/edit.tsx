...
export default function SettingsEdit({ settings, flash }: SettingsPageProps) {
    const { data, setData, put, processing, errors, progress } = useForm<SettingsForm>({
    ...
    const submit = (event: React.FormEvent) => {
        ...
        put(route('admin.contact.settings.update'), {
            forceFormData: true,
            preserveScroll: true,
        });
    };

    const showFile = (key: string) => (file: File | null) => file?.name ?? null;
            ...
            <form onSubmit={submit} className="flex h-full flex-1 flex-col gap-6 p-4 md:p-6">
                ...
                <div className="grid gap-6">
                    <Section title="Hero" description="The top section visitors see first.">
                        ...
                        <TextField label="Highlight" value={str(data.hero_highlight)} onChange={setStr('hero_highlight')} error={errors.hero_highlight} hint="Accented part of the title." />
                        <TextField label="Background image" value={str(settings.hero_background_image)} onChange={() => {}} disabled hint="Kept until you upload a replacement." />
                        <div className="sm:col-span-2">
                            <label className="text-sm font-medium">
                                ...
                                <input
                                    ...
                                    className="mt-2 flex h-10 w-full cursor-pointer items-center text-sm text-muted-foreground file:mr-3 file:cursor-pointer file:border-0 file:bg-muted file:px-4 file:py-2 file:text-sm file:font-medium"
                                />
                            </label>
                            {data.hero_background_image instanceof File && (
                                <p className="mt-1 text-xs text-muted-foreground">New file: {showFile('hero_background_image')(data.hero_background_image)}</p>
                            )}
                            {errors.hero_background_image && <p className="mt-1 text-sm text-red-600 dark:text-red-400">{errors.hero_background_image}</p>}
                        </div>
                        <div className="sm:col-span-2">
                        ...
                        <TextField label="Primary button text" value={str(data.hero_primary_button_text)} onChange={setStr('hero_primary_button_text')} error={errors.hero_primary_button_text} />
                        <TextField label="Primary button link" value={str(data.hero_primary_button_link)} onChange={setStr('hero_primary_button_link')} error={errors.hero_primary_button_link} hint="Defaults to the contact form." />
                        <TextField label="Secondary button text" value={str(data.hero_secondary_button_text)} onChange={setStr('hero_secondary_button_text')} error={errors.hero_secondary_button_text} />
                        <TextField label="Secondary button link" value={str(data.hero_secondary_button_link)} onChange={setStr('hero_secondary_button_link')} error={errors.hero_secondary_button_link} />
                    </Section>
                    ...
                    <Section title="Contact form" description="Headings shown beside the contact form.">
                        ...
                        <TextField label="Success message" value={str(data.form_success_message)} onChange={setStr('form_success_message')} error={errors.form_success_message} />
                        <div className="sm:col-span-2">
                            <TextAreaField label="Form description" value={str(data.form_description)} onChange={setStr('form_description')} error={errors.form_description} />
                        </div>
                    </Section>
                    ...
                    <Section title="FAQ section" description="Settings for the frequently asked questions section.">
                        ...
                        <div className="sm:col-span-2">
                            <TextAreaField label="Description" value={str(data.faq_description)} onChange={setStr('faq_description')} error={errors.faq_description} />
                        </div>
                    </Section>
                    ...
                    <Section title="Team section" description="Settings for the team members section.">
                        ...
                        <div className="sm:col-span-2">
                            <TextAreaField label="Description" value={str(data.team_description)} onChange={setStr('team_description')} error={errors.team_description} />
                        </div>
                    </Section>
                    ...
                    <Section title="Location section" description="Settings for the offices section.">
                        ...
                        <div className="sm:col-span-2">
                            <TextAreaField label="Description" value={str(data.location_description)} onChange={setStr('location_description')} error={errors.location_description} />
                        </div>
                    </Section>

                    <Section title="Live chat" description="Headings shown in the chat widget.">
                        <TextField label="Live chat title" value={str(data.live_chat_title)} onChange={setStr('live_chat_title')} error={errors.live_chat_title} />
                        <div className="sm:col-span-2">
                            <TextAreaField label="Live chat description" value={str(data.live_chat_description)} onChange={setStr('live_chat_description')} error={errors.live_chat_description} />
                        </div>
                    </Section>
                    ...
                    <Section title="SEO" description="Search engine and social sharing metadata.">
                        ...
                        <TextField label="Open Graph image" value={str(settings.og_image)} onChange={() => {}} disabled hint="Kept until you upload a replacement." />
                        <div className="sm:col-span-2">
                            <label className="text-sm font-medium">
                            ...
                            {data.og_image instanceof File && (
                                <p className="mt-1 text-xs text-muted-foreground">New file: {showFile('og_image')(data.og_image)}</p>
                            )}
                    ...
                    <Card>
                        <CardContent className="pt-6">
                            <CheckboxField label="Page is active" checked={Boolean(data.is_active)} onChange={(value) => setData('is_active', value)} hint="When disabled, the public contact page hides." />
                        </CardContent>
                    </Card>
                </div>

                <div className="sticky bottom-4 flex items-center justify-end gap-3 rounded-lg border bg-card p-3 shadow-lg">
                    {progress && <p className="text-sm text-muted-foreground">Uploading {Math.round(progress.percentage ?? 0)}%</p>}
