...
export default function ContactMap({ location }: ContactMapProps) {
    ...
    return (
        <div className="bg-muted relative h-80 w-full overflow-hidden border sm:h-[420px] lg:h-full lg:min-h-[460px]">
            {embedUrl && active ? (
                <iframe
                    src={embedUrl}
                ...
                <div className="flex size-full flex-col items-center justify-center gap-6 p-8 text-center">
                    ...
                    <div className="flex flex-wrap items-center justify-center gap-x-8 gap-y-3">
                        {embedUrl && (
                            <Button variant="outline" className="rounded-sm" onClick={() => setActive(true)}>
                                Load map
                            </Button>
                        )}
                        {linkUrl && (
